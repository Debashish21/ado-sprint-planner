...
import { getPlan, getDeferredSet, getStatusMap } from "./lib/storage";
import { orderItems, projectPlan, remainingWorkingDays } from "./lib/plan";
import { LocalStatus, Preferences, WorkItem } from "./lib/types";
...
export default function SprintMenuBar() {
  ...
  const { data, isLoading } = useCachedPromise(
    ...
    const [iteration, items, plan, statusMap, deferred] = await Promise.all([
      getCurrentIteration(),
    ...
    const capacity =
      parseInt(getPreferenceValues<Preferences>().dailyCapacity || "3", 10) || 3;
    const days = remainingWorkingDays(iteration.startDate, iteration.finishDate);
  ...
  const title = isLoading ? undefined : `${remaining.length}`;
